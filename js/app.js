...
let veryOldInventors = inventors.filter(
  (person) => person.year < 1600 && person.year >= 1500
);
...
let inventorNamedAda = inventors.find((person) => person.first === "Ada");
...
let firstLast = people.map((person) => {
  let splitStr = person.split(",").reverse().join(" ");
  return splitStr;
});
...
let isAdultPresent = devs.some((dev) => dev.year <= 2006);
// Check your return value:
...
let isEveryone19OrOlder = devs.every((dev) => dev.year <= 2005);
...
// Enter your solution code here:
let totalYearsLived = inventors
  .map((person) => {
    return person.passed - person.year;
  })
  .reduce((acc, value) => acc + value, 0);
// let yearsLived = acc.passed - acc.year;
// console.log(yearsLived)
// console.log(acc.passed - acc.year);
// return yearsLived + value;
...
options:
1) I need to create an object
2. Dynamically add the number of times that object appears in the array to the tally of the obj
3. Create a class that will contain a constructor that makes the travel options object
then as the array is iterated and it encouters a key that matches it will update that count
